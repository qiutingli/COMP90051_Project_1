...
def initialize_featured_data_frame():
    return pd.DataFrame(columns = featured_df_columns)


# Construct featured data frame iterating the original data.
def construct_featured_data_frame(featured_df):

    for i in range(len(original_training_data))[1:10]:
        tweet = original_training_data.iloc[i]['tweet']
        ...
        featured_dict = {
            featured_df_columns[0]: original_training_data.iloc[i]['id'],
            featured_df_columns[1]: retweet_or_not,
            featured_df_columns[2]: num_of_words,
            featured_df_columns[3]: num_of_hashtags,
            featured_df_columns[4]: hashtag_contents,
            featured_df_columns[5]: num_of_mentions,
            featured_df_columns[6]: num_of_urls,
            featured_df_columns[7]: url_contents
        }
        featured_df = featured_df.append(featured_dict, ignore_index=True)

    return featured_df

def write_featured_df_to_csv(featured_df, file):
    if os.path.exists(file):
        featured_df.to_csv(file)
    else:
        try:
            os.makedirs(file)
        except Exception as e:
            print(e)
        featured_df.to_csv(file, index=False, encoding="utf-8")
...
if __name__ == '__main__':
    ...
    featured_df_columns = ['id', 'retweet_or_not', 'num_of_words', 'num_of_hashtags', 'hashtag_contents',
                           'num_of_mentions', 'num_of_urls', 'url_contents']
    featured_df = initialize_featured_data_frame()
    featured_df = construct_featured_data_frame(featured_df)

    featured_training_df_path = "%s/data/featured_training_df.csv" % os.path.abspath('.')
    write_featured_df_to_csv(featured_df, featured_training_df_path)
